...
def get_preferred_llm_class(config_path: str = "mcp_agent.secrets.yaml") -> Type[Any]:
    ...
    Reads from YAML config file and returns GeminiAugmentedLLM if api_key
    is available, otherwise raises an error.
    ...
    try:
        ...
        if os.path.exists(config_path):
            ...
            # Check for gemini API key (Gemini only check)
        ...
        print(f"🤖 CRITICAL ERROR in LLM selection: {e}")
        # Raise an explicit error to prevent silent failure
        raise ValueError("Could not initialize Gemini LLM. Check API key and configuration.")
